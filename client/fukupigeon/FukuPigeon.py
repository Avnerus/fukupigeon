...
class Food(object):
    def __init__(self, x, y, z):
        ...
        print ('Spawning a food in {0}, {1}, {2}'.format(self.x, self.y, self.z))
...
class Vehicle(object):
    def __init__(self, x, y, z):
        ...
        print ('Spawning a vehicle in {0}, {1}, {2}'.format(self.x, self.y, self.z))
...
class Aeroplane(object):
  def __init__(self, model, recalc_time, refid):
    ...
    self.x, self.y, self. z = 0.0, 30.0, 0.0
  ...
  def shoot(self, target):
    ...
    if self.seq_b < self.num_b:
      return 0.0
    print ('Shooting to {0}, {1}, {2}'.format(target[0], target[1], target[2]))
    #animate bullets
    self.seq_b = 0
    #check for hit
...
pi3d.Light(lightpos=(1, -1, -3), lightcol =(1.0, 1.0, 1.0), lightamb=(1.0, 1.0, 1.0))
...
mapdepth = 10000.0
mapheight = 10.0
mountimg1 = pi3d.Texture("textures/road1.jpg")
bumpimg = pi3d.Texture("textures/road1.jpg")
...
while DISPLAY.loop_running() and not inputs.key_state("KEY_ESC"):
  ...
  if counter % 200 == 0:
      ...
      if len(foods) > 0:
          foods.pop()
      newFood = Food(loc[0] + random.randint(-10, 10), loc[1], loc[2] -200) 
